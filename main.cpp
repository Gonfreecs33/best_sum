...
int main() {
    ...
    auto t_map = benchmark([](auto f1, auto f2) {
        map_version::PiecewiseLinearFunction g1, g2;
        g1 = map_version::delta_profile(5, 10, 20, 30);
        g2 = map_version::cba_profile(10, 15, 25);
        g1.sum(g2);
        return g1;
    });

    // Benchmark version "list" avec add
    auto t_list = benchmark([](auto f1, auto f2) {
        f1.add(f2);
        return f1;
    });
